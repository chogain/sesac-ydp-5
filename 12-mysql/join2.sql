...
/*
이 때, Inner Join 뒤에 ON 키워드를 통해 일치하는 속성을 기준으로 결합할 수도 있으며, 이를 Equi Join(동등 join)이라고 한다.
*/
SELECT * FROM instructor I JOIN teaches T ON I.id=T.id;
-- instructor I: 이후부터는 instructor 테이블을 I라고 지칭하겠다.
-- teaches T: 이후부터는 teaches 테이블을 T라고 지칭하겠다.
...
select * from instructor; -- 4개
select * from teaches; -- 4개
